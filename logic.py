...
class TrackerLogic:
    ...
    It does not know anything about Tkinter.
    """
    ...
    def add_record(self, record_date, count):
        ...
        self.database.add_or_update_record(record_date, count)
    ...
    def update_record(self, old_date, new_date, count):
        ...
        if self.database.get_record(old_date) is None:
            raise ValueError("The record you are trying to edit does not exist.")

        if old_date != new_date and self.database.get_record(new_date) is not None:
            raise ValueError("A record already exists for the new date.")

        self.database.update_record(old_date, new_date, count)

    def delete_record(self, record_date):
        self._validate_date(record_date)
        return self.database.delete_record(record_date)

    def get_record(self, record_date):
        self._validate_date(record_date)
        return self.database.get_record(record_date)

    def get_records(self):
        return self.database.get_records()

    def get_total(self):
        return sum(self.get_records().values())
    ...
    def _validate_date(self, record_date):
        if not isinstance(record_date, date):
            raise TypeError("record_date must be a datetime.date object.")
